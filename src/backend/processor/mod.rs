...
use crate::{
    backend::{engine::EngineOutput, pool::{Fish, Pool}},
    ipc::SimulationData,
};
...
pub trait SimulationDataAdapter: TryFrom<SimulationData> + Send {
    ...
    fn to_simulation_data(self, prev_state: &SimulationData, engine_output: &EngineOutput) -> Result<SimulationData, Self::AdapterError>;
}
    ...
    I: Fn() -> A,
{
    ...
    pub fn process_engine_output(&self, prev_state: &SimulationData, engine_output: &EngineOutput) -> Result<SimulationData, A::AdapterError> {
    ...
    where 
        ...
        NI: Fn() -> NA
    {
        SimulationOutputProcessor { adapters_pool: self.adapters_pool, adapter_initializer: new_initializer }
    }
}
