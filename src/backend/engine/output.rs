...
impl EngineOutput {
    ...
    ///
    /// # Arguments:
    /// * `grid_spacing` - Distance between one edge of a cell to another one.
    ...
    pub fn new(grid_spacing: f64, particles_count: usize, width: usize, height: usize) -> Self {
        ...
        for x in 1..(width - 1) {
            for y in 1..(height - 1) {
                ...
                let x_increment = grid_spacing * (particles_in_this_cell + 1.0).recip();
